...
const s = (html: string) => `<div style="padding:28px 32px;">${html}</div>`;
const h1 = (t: string) => `<h1 style="margin:0 0 8px;font-size:18px;font-weight:600;color:#1a1916;letter-spacing:-0.3px;">${t}</h1>`;
const p = (t: string) => `<p style="margin:0 0 16px;font-size:14px;color:#6b6a64;line-height:1.6;">${t}</p>`;
...
const tr = (label: string, val: string) =>
  `<tr><td style="padding:5px 0;font-size:12px;color:#999891;width:140px;">${label}</td><td style="padding:5px 0;font-size:12px;color:#1a1916;font-weight:500;">${val}</td></tr>`;
...
export async function emailRevisionAdmin({ adminEmail, adminNombre, contratistaNombre, entregableNombre, contratoTitulo, valor, url }: {
  ...
  return resend.emails.send({
    ...
    html: baseLayout(s(`
      ...
      ${p(`<strong>${contratistaNombre}</strong> ha enviado un entregable para tu revisión.`)}
      ${hr()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${tr("Entregable", entregableNombre)}
        ${tr("Contrato", contratoTitulo)}
        ${tr("Valor", `$${valor.toLocaleString("es-CO")}`)}
        ${tr("Contratista", contratistaNombre)}
        ${tr("Estado", chip("En revisión", "ambar"))}
      </table>
...
export async function emailAprobado({ contratistaNombre, contratistaEmail, entregableNombre, contratoTitulo, valorOriginal, valorFinal, penalizacion, bono, retencion, retencionPorcentaje, url }: {
  ...
  retencion: number; retencionPorcentaje: number; url: string;
}) {
  return resend.emails.send({
    ...
    html: baseLayout(s(`
      ...
      ${p(`Tu entregable <strong>${entregableNombre}</strong> fue aprobado. El pago será procesado pronto.`)}
      ${hr()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${tr("Entregable", entregableNombre)}
        ${tr("Contrato", contratoTitulo)}
        ${tr("Valor base", `$${valorOriginal.toLocaleString("es-CO")}`)}
        ${penalizacion > 0 ? tr("Penalización", `<span style="color:#a02020;">-$${penalizacion.toLocaleString("es-CO")}</span>`) : ""}
        ${bono > 0 ? tr("Bono anticipado", `<span style="color:#1a7a4a;">+$${bono.toLocaleString("es-CO")}</span>`) : ""}
        ${retencion > 0 ? tr(`Retención (${retencionPorcentaje}%)`, `<span style="color:#92600a;">-$${retencion.toLocaleString("es-CO")}</span>`) : ""}
        ${tr("Neto a recibir", `<strong style="color:#1a7a4a;">$${valorFinal.toLocaleString("es-CO")}</strong>`)}
        ${tr("Estado", chip("Aprobado", "verde"))}
      </table>
...
export async function emailRechazado({ contratistaNombre, contratistaEmail, entregableNombre, contratoTitulo, comentario, url }: {
  ...
  return resend.emails.send({
    ...
    html: baseLayout(s(`
      ...
      ${p(`Tu entregable <strong>${entregableNombre}</strong> fue rechazado. Revisa los comentarios y vuelve a enviarlo.`)}
      ${hr()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${tr("Entregable", entregableNombre)}
        ${tr("Contrato", contratoTitulo)}
        ${tr("Estado", chip("Rechazado", "rojo"))}
      </table>
...
export async function emailRecordatorio({ contratistaEmail, contratistaNombre, entregableNombre, contratoTitulo, fechaLimite, diasRestantes, valor, url }: {
  ...
  entregableNombre: string; contratoTitulo: string;
  fechaLimite: Date; diasRestantes: number;
  valor: number; url: string;
}) {
  ...
  return resend.emails.send({
    ...
    html: baseLayout(s(`
      ...
      ${p(`${mensajeDias}. Asegúrate de enviar tu entregable a revisión antes de la fecha límite.`)}
      ${hr()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${tr("Entregable", entregableNombre)}
        ${tr("Contrato", contratoTitulo)}
        ${tr("Valor", `$${valor.toLocaleString("es-CO")}`)}
        ${tr("Fecha límite", fechaFormateada)}
        ${tr("Tiempo restante", chip(
